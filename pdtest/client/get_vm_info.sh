		...
		do
			...
			then
				...
				then
					...
					do
						if [ "$(echo ${vlan_ids[$num]} | awk -F"," '{ for(i=1;i<=NF;i++) { if($i==vlan_id) {print 0; break;} } }' vlan_id=$vm_pvid)" == "0" ]
						then
...
get_pv_uniqueid_info() {
	...
	do
		...
		# if [ "$(echo $?)" != "0" ]
		# then
				# pd_error "$uniqueid[$pv_i]" "1000038"
		# fi
		pv_i=$(expr $pv_i + 1)
	done
}
			...
			do
				...
				then
					...
					# done
						...
						disk_name=$(echo "$disk_map_info" | grep "\-C${vios_scsi_slot}:") 
						len=0
						...
						vm_lv_num[${length}]=$(echo "$lv_map_info" | grep "\-C${vios_scsi_slot}:" | awk -F":" '{print NF-1}')
						vm_lv[${length}]=$(echo "$lv_map_info" | grep "\-C${vios_scsi_slot}:" | awk -F":" '{for(i=2;i<=NF;i++) print $i}')
						vm_pv_num[${length}]=$(echo "$pv_map_info" | grep "\-C${vios_scsi_slot}:" | awk -F":" '{print NF-1}')
						vm_pv[${length}]=$(echo "$pv_map_info" | grep "\-C${vios_scsi_slot}:" | awk -F":" '{for(i=2;i<=NF;i++) print $i}')
						vm_lu_num[${length}]=$(echo "$lu_map_info" | grep "\-C${vios_scsi_slot}:" | awk -F":" '{print NF-1}')
						vm_lu[${length}]=$(echo "$lu_map_info" | grep "\-C${vios_scsi_slot}:" | awk -F":" '{for(i=2;i<=NF;i++) print $i}')
